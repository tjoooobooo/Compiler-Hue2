...
class EnvImpl extends StaticEnv {
  ...
  override val define: PartialFunction[(String, ProgSymbol), ProgSymbol] = {
    case (name, symb) if ! actualFrame.isLocallyDefined(name) =>
      actualFrame.define(name, symb)
      symb
  }

  override val defineVariable: PartialFunction[String, VarSymbol] = {
  ...
  override val defineProcedure: PartialFunction[String, ProcSymbol] = {
    case name if ! actualFrame.isLocallyDefined(name) =>
  ...
  override val defineValParam: PartialFunction[String, ValParamSymbol] = {
    case name if ! actualFrame.isLocallyDefined(name) =>
  ...
  override val defineRefParam: PartialFunction[String, RefParamSymbol] = {
    case name if ! actualFrame.isLocallyDefined(name) =>
